...
const (
	appName = "smartsync-shared-files"
...
// mockgen -destination mocks/mock_restAdapter.go -package mocks -mock_names RestAdapter=MockRestAdapter openappsec.io/smartsync-shared-files/internal/app RestAdapter
...
// FileSystemDriven defines the Adapter control API
type FileSystemDriven interface {
	health.Checker
...
func NewApp(adapter RestAdapter, conf Configuration, healthSvc HealthService) *App {
	return &App{
		httpDriver: adapter,
		conf:       conf,
		health:     healthSvc,
...
func (a *App) Stop(ctx context.Context) error {
	...
	if err := a.httpDriver.Stop(ctx); err != nil {
		errorsArr = append(errorsArr, errors.Wrap(err, "Failed to gracefully shutdown server"))
	}
	if len(errorsArr) == 0 {
		return nil
	}
	return errors.Errorf("Failed to stop service. errors: %v", errorsArr)
}

func (a *App) healthInit() {
	// add readiness checks for all external services that supposed to implement AddReadinessChecker
	// for example: a.health.AddReadinessChecker(a.dbRepo)
	return
}
